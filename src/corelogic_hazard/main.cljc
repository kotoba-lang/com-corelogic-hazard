(ns corelogic-hazard.main
  "Kotodama WASM entrypoint for the Corelogic_hazard clean-room actor (L4) — Clojure port.
  ...
  facts (`corelogic_hazard.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "corelogic_hazard")
(def tier "L4")
...
(def entity-specs
  [{:entity "Account"     :plural "accounts"     :id-prefix "corelogi_acc"
    :fields [:holderId :type :balance :currency :status] :required [:type :balance]
    :coerce {:balance :float}           :refs {}}
   {:entity "Loan"        :plural "loans"        :id-prefix "corelogi_loa"
...
(defn emit-facts
  "EAVT facts for one record: {\"corelogic_hazard.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "corelogic_hazard-compat" :tier tier :entities entities} 200])
...
(defn start! [] :corelogic_hazard-compat/ready)
