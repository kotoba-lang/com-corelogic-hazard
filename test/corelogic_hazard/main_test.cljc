(ns corelogic-hazard.main-test
  "Contract + behavioral test for the corelogic-hazard L4 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "corelogic_hazard-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'corelogic-hazard.main-test)]
